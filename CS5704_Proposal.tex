...
To detect cloned codes, the first thing is to know clone types: there are two main kinds of similarity between code fragments. The first kind has textual similarity, and often comes from copying and pasting. Textual similarity can fall into three types: Type 1 simply includes the variations in whitespace, layout and comments. In addition to Type 1, Type 2 allows more variations in identifiers, literals and types. Type 3 contains Type 2 and allows further modifications such as changed, added or removed statements. An example of Type 1, 2, 3 clones is shown in Fig \ref{fig:TOC}. 
The second main kind is functional similarity. Functional similarity is described by Type 4 clone, or semantic clone, which is more complicated. In this case code fragments perform the same computation but are implemented in different ways. Few tools have been developed to detect Type 4 clones so far.\\


\begin{figurehere}
\centering \includegraphics[width=0.4\textwidth]{types} \caption{Types of Clone} \label{fig:TOC}
\end{figurehere}

To detect the code clone, researchers have developed different techniques and tools. The first kind is based on textual comparison, which means comparing whole lines to each other textually. The second kind is based on tokens, 

trees and graphs. Among all the techniques and tools, token-based method performs well in both clone type detection and time cost.
...
CCFinder is a classical tool, it compares each code portion so results in a high accuracy, however that also results in huge time cost. 
CMCD uses Count Matrix, which works well in extracting variables those are copied but in different names, but is doesn't include other contributions. 
Boreas uses CMCD to compare variables, as well as taking keywords and symbols into account, but still has room for improvement, both executing time, and detection accuracy. 
RTF uses flexible tokenization, but also loses a lot of imformation.
